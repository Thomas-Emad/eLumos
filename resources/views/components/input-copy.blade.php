...
<div class="relative">
    ...
    <button data-copy-to-clipboard-target="{{ $id }}-url" data-tooltip-target="tooltip-{{ $id }}-url"
        ...
        <span id="success-icon-{{ $id }}-url" class="hidden  items-center">
    ...
    <div id="tooltip-{{ $id }}-url" role="tooltip"
        class="absolute z-10 invisible inline-block px-3 py-2 text-sm font-medium text-white transition-opacity duration-300 bg-gray-900 rounded-lg shadow-sm opacity-0 tooltip dark:bg-gray-700">
        <span id="default-tooltip-message-{{ $id }}-url">Copy to clipboard</span>
        <span id="success-tooltip-message-{{ $id }}-url" class="hidden">Copied!</span>
