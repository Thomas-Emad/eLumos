                ...
                <div class="p-4 flex justify-between items-center flex-col md:flex-row gap-4">
                    ...
                    <div class="flex flex-col gap-4 items-center">
                        <p class='text-gray-800 text-center'>
                            @php
                                $examStudentt = $currentLecture->examStudentt->last();
                            @endphp
                            @if (is_null($examStudent) || $examStudent?->status == 'processing')
                                ...
                                <span class="text-xs"><b>Note: </b>You will not get the certificate without solving all the
                                    exams</span>
                            @elseif(!is_null($examStudent) && $examStudent?->status == 'sucess')
                                <span> <b>Congratulations</b>, you have successfully passed this exam, you can move
                                    on.</span>
                            @elseif(!is_null($examStudent) && $examStudent?->status == 'failed')
                                <b>You failed the exam,</b> <br> <span> But don't worry, you can try again anytime you
                                    want..</span>
                            @elseif(!is_null($examStudent) && $examStudent?->status == 'waiting')
                        ...
                        <div class="flex gap-2 items-center">
                            ...
                            @if (is_null($examStudent) || in_array($examStudent?->status, ['processing', 'sucess', 'failed']))
                                <form action="{{ route('dashboard.student.exams.store') }}" method="POST">
                                    @csrf
                                    <input type="hidden" name="type"
                                        value="{{ is_null($examStudent) ? 'join' : 'retest' }}">
